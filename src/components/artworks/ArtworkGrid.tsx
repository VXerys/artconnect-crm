          ...
          >
            ...
            <div className="aspect-[4/3] relative overflow-hidden">
              ...
              <div className="absolute top-3 left-3 right-3 flex items-start justify-between">
                ...
                {/* Year badge */}
                <span className="px-2 py-1 rounded-lg text-xs font-medium bg-background/80 backdrop-blur-md border border-border flex items-center gap-1">
              ...
              <div className="absolute inset-0 flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-all duration-300">
                ...
                <Button 
                  variant="secondary" 
                  size="icon"
                  className="h-12 w-12 rounded-full bg-background/90 backdrop-blur-md hover:bg-primary hover:text-primary-foreground transition-all duration-200 shadow-lg"
                  onClick={(e) => {
                    e.stopPropagation();
                    onView(artwork);
                  }}
                ...
                <Button 
                  variant="secondary" 
                  size="icon"
                  className="h-12 w-12 rounded-full bg-background/90 backdrop-blur-md hover:bg-destructive hover:text-destructive-foreground transition-all duration-200 shadow-lg"
                  onClick={(e) => {
                    e.stopPropagation();
                    onDelete(artwork);
                  }}
                ...
                {artwork.price ? (
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-semibold text-primary">
                      {new Intl.NumberFormat('id-ID').format(artwork.price)}
                    </span>
                  </div>
                ) : (
                  <span className="text-sm text-muted-foreground italic">Belum ada harga</span>
                )}
              </div>
            </CardContent>
